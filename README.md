...
- 💾 **数据持久化**：任务数据自动保存到本地
- ⏰ **实时时钟**：显示当前时间
- 📊 **准确率统计**：预计时长与实际有效时长的比率分析
- ⏱️ **准时率统计**：预计时长与实际总耗时的比率分析
...
| 指标 | 公式 | 颜色规则 |
|------|------|----------|
| 准确率 | 预计时长 / 有效时长 × 100% | < 100% 红色，= 100% 绿色，> 100% 蓝色 |
| 准时率 | 预计时长 / 实际时长 × 100% | < 100% 红色，= 100% 绿色，> 100% 蓝色 |

### 重新计算接口

当算法更新后，可通过以下方式重新计算所有历史数据：
...
该接口会重新计算所有记录的准确率和准时率，并自动保存到服务器。
...
- Go 1.21 或更高版本（需要 Go 1.16+ 支持 embed 特性）

### 静态资源嵌入机制

项目使用 Go 1.16+ 的 `embed` 特性，编译时会自动将 `html/` 目录下的所有文件嵌入到可执行文件中：

- **编译时**：`//go:embed html/**` 指令将静态资源打包进二进制文件
- **运行时**：程序自动将嵌入的文件解压到 `web/` 目录提供服务

这样部署时只需一个可执行文件，无需携带额外的静态资源文件夹。
